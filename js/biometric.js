...
async function _enrollBiometric() {
  ...
  try {
    ...
    if (credential) {
      ...
      // ── Optional: send public key to your backend for full verification
      // await fetch('/api/webauthn/register', {
      //   method: 'POST',
      //   headers: { 'Content-Type': 'application/json',
      //              'Authorization': 'Bearer ' + localStorage.getItem('lmr_token') },
      //   body: JSON.stringify({ credentialId: credId, userId: user.id })
      // });
    }
...
async function _authenticateBiometric() {
  ...
  try {
    ...
    if (assertion) {
      // ── Full production flow: send assertion to backend for verification
      // const res = await fetch('/api/webauthn/authenticate', {
      //   method: 'POST',
      //   headers: { 'Content-Type': 'application/json' },
      //   body: JSON.stringify({ assertionId: _bufferToBase64(assertion.rawId) })
      // });
      // const { token, user } = await res.json();
      // Auth.saveSession(token, user);

      // ── Local session restore (works offline / during backend setup) ──
      const savedUser = JSON.parse(localStorage.getItem(BIO_USER_KEY) || '{}');
      const token     = localStorage.getItem('lmr_token');

      if (savedUser?.email) {
        APP.user    = savedUser;
        APP.unit_id = savedUser.unit_id || null;

        toast('Welcome back, ' + (savedUser.name?.split(' ')[0] || 'there') + '! 👋', 'success');

        // Try to load live data; fall through gracefully if offline
        try {
          const data = await Auth.me();
          APP.user    = data.user;
          APP.unit_id = data.user.units?.[0]?.unit_id || null;
        } catch { /* offline — use cached state */ }

        if (APP.unit_id) {
          showDashboard();
        } else {
          showScreen('login-screen'); toast('Please log in first', 'info');
        }
      } else {
