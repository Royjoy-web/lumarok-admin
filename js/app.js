...
async function submitCreateUnit() {
  ...
  try {
    ...
    showToast(`Unit created: ${unit.unit_id}`, 'success');
...
async function submitCreateInstaller() {
  ...
  setLoading(btn, true);
  try {
    await Admin.createInstaller({ name, email, password: pass });
    showToast(`Installer ${email} created`, 'success');
